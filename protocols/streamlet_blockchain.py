...
class StreamletBlockchain:
    ...
    def finalize(self):
        ...
        if (found_three_in_a_row is not None):
            ...
            self.blockchain_top = [x for x in self.blockchain_top if
                                   x.leads_to_block(found_three_in_a_row) or x in self.finalized]
            self.blockchain_all = [x for x in self.blockchain_all if
                                   x.leads_to_block(found_three_in_a_row) or x in self.finalized]
...
class StreamletBlock:
    ...
    def __eq__(self, other):
        if (other is None):
            return False
        return self.prev_block == other.get_prev_block() and \
               self.epoch_num == other.get_epoch_num() and self.transactions == other.get_transactions()
